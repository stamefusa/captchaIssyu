...
class CaptchaBuilder implements CaptchaBuilderInterface
{
    ...
    protected function writePhrase($image, $phrase, $font, $width, $height)
    {
        ...
        $x = $initialX + $this->rand(0, 3);
        //$y = ($height - $textHeight) / 2 + $size;
        $y = $height / 2;
        //$x = $width - (int)($textWidth / $length) - 40 + $this->rand(0, 10);
        ...
        for ($i=0; $i<$length; $i++) {
            ...
            //$x += $w;
            if (($i+1)%$halfNum === 0 && ($i+1)/$halfNum < 2) {
                $x = $initialX + $this->rand(-3, 3);
                $y += ($box[1] - $box[7] + $this->rand(-3, 5));
                //$x -= ($w + $this->rand(-1, 20));
                //$y = $initialY + $this->rand(0, 3);
            } else {
                //$y += box[1] - $box[7];
                $x += $w + $this->rand(-5, 3);
                $y += $this->rand(-1, 2);
    ...
    public function build($width = 150, $height = 40, $font = null, $fingerprint = null)
    {
        ...
        if ($font === null) {
            ...
            $font = __DIR__ . '/Font/ipa'.$this->rand(0, 1).'.ttf';
        }
        ...
        // Write CAPTCHA text
        if ($this->isVertical) {
            $color = $this->writePhrase2($image, $this->phrase, $font, $width, $height);
        } else {
